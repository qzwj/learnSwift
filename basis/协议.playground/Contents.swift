...
class SomeSubClass: SomeSuperClass, initProtocol1 {
    required override init() { // 因为遵守协议要加上required, 继承自弗雷,要加上override
...
protocol initProtocol2 {
    init?(a: Int)
}
class SomeClass1: initProtocol2 {
    var a: Int
    required init?(a: Int){
        if a == 0 {
            return nil
...
var a: SomeClass1? = SomeClass1(a: 0) //可失败构造器一定可选类型的
a?.a
a?.a = 2
a = SomeClass1(a: 1)
a?.a
...
//隐式解包可失败构造器
protocol initProtocol3 {
    init!(s: Int)
}
class SomeClass2: initProtocol3 {
    var s: Int
    required init!(s: Int) {
        self.s = s
    }
}
var b: SomeClass2 = SomeClass2(s: 4)
...
class SnakeAndLadders: DiceGame {
    ...
    init() {
        ...
        board[14] = -10; board[19] = -11; board[22] = -02; board[25] = -08
    }
    ...
    func play() {
        ...
        gameLoop: while square != finalSquare {
            ...
            case finalSquare: //这种符合25, 否则一直循环
                break gameLoop
            case let newSquare where newSquare > finalSquare:
                print("newSquare is \(newSquare)")
                continue gameLoop
            default:
                square += diceRoll
                square += board[square]
...
print("d12 = \(d12.textualDescription)")
...
extension SnakeAndLadders: PrettyTextRepresentable {
    var prettyTextualDescription: String {
        var output = textualDescription + ":\n" //这里textualDescription之前扩展过, 所以可以直接用, 否则要在这里声明实现
...
//struct ss: SomeClassOnlyProtocol { //报错
//
//}
//注意
...
let birthdayPerson = Person1(name: "dqy", age: 23)
...
let seattle = City(latitude: 11.2, longitude: 22.2, name: "Seattle")
...
let objects: [AnyObject] = [
    Circle(radius: 2.0),
    Country(area: 3234_222),
    Animal(legs: 4)
]
for object in objects {
    if let objectWithArea = object as? HasArea { // as? 返回?后面类型的一个可选类型
...
@objc class TowardsZeroSource: NSObject, CounterDataSource { //@objc 一定要继承NSObject类型
...
//let generator1 = LinearConguentialGenerator()
print("Here's a random number: \(generator.random())")
