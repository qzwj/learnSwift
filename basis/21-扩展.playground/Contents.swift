...
//这个新的构造器,首先根据提供的 center 和 size 的值计算一个合适的原点。然后调用该结构体的逐一成员构造器 init(origin:size:)，该构造器将新的原点和大小的值保存到了相应的属性中：
let centerRect = Rect(center: Point(x: 4.0, y: 4.0), size: Size(width: 3.0, height: 3.0))
print("origin.x = \(centerRect.origin.x), origin.y = \(centerRect.origin.y)")
...
printIntegerKinds([3,19,-28,0,-6,11])
